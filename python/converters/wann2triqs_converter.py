...
class Wannier2TRIQS_converter(Check,ConverterTools):
    """
        ...
        **Functionality**::
            ...
            -  Reads projectors from seedname*.chk file(s) and transforms it to the proper format
                (method: _produce_projectors, fortran module: read_chkpt)
    ...
    def __init__(self, filename=None, density_required=None,
        ...
        self.n_inequiv_shells, self.corr_to_inequiv, self.inequiv_to_corr=self.det_shell_equivalence(corr_shells=self.corr_shells)
    ...
    def __read_parameters_from_h5_file(self, subgrp=None, thing_to_load=None,just_check=True):
                ...
                if isfile(self._filename + ".h5"):
                    ...
                    if subgrp not in ar:
                        ...
                        for it in thing_to_load:
                            ...
                            if it in ar[subgrp]:
                                if not just_check:
                                    exec "self._%s = ar['%s'][it]" % (it, subgrp)
                            else:
                                if not just_check:
        ...
        if found_all and not just_check:
            ...
            for it in thing_to_load:
                exec "self._%s = bcast(self._%s)" % (it, it)
    ...
    def n_orbitals(self):
        ...
        :return: Number of bands for each k-point
        """
        return self.n_orbitals


    @n_orbitals.setter
    def n_orbitals(self,val=None):
       self.report_error("Value of n_orbitals cannot be overwritten!")
