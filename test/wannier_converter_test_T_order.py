...
#    Test for validity of constructing T matrix for the given order of initial correlated projections from  *.win file.
#
#    Input files:  wannier_converter_T_d.win, wannier_converter_T_d_long.win, wannier_converter_T_d_other.win,
#                  wannier_converter_T_d_wien2k.win, wannier_converter_T_f.win, wannier_converter_test_T_order.py
#
#    Output files: wannier_converter_test_T_order.output.h5
#
#    Command: pytriqs wannier_converter_test_T_order.py
#
#    Description:
...
def benchmark(filename=None):
    ...
    # create an converter object
    converter = Wannier90Converter(filename=filename)

    # read keywords from filename.win file
    converter._read_win_file()
    
    # add manually sort to shells 
    #
    # In the real calculation  we need *seedname_hr.dat 
    # file in this step.  Please keep in mind that whether sort is evaluated properly is tested in another test.
    ...
    # evaluate correlated shells
    converter._get_corr_shells()

    # evaluate number of correlated shells
    converter.n_corr_shells = len(converter.corr_shells)
    
    # evaluate inequivalent correlated shells and mapping from
    ...
    # evaluate T according to the order of initial projections found in *win file
    converter._get_T()

    # shorten name of the directory in hdf file
    folder_name = filename.replace("wannier_converter", "").strip()

    # save data
    ar = HDFArchive('wannier_converter_test_T_order.output.h5', 'a')
